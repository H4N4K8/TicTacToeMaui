...
namespace TicTacToeMaui
{
    public partial class MainPage : ContentPage
    {
        ...
        async void AnimateTitle()
        {
            ...
            await Task.WhenAll(
            Christmas.ScaleTo(.5, 1000), // Scale down
            Christmas.RotateTo(360, 1000) // Rotate 360 degrees
            );

            // Reverse animation
            await Task.WhenAll(
            Christmas.ScaleTo(1, 1000), // Scale back to normal
            Christmas.RotateTo(0, 1000) // Reset rotation
            );
            // Fades the title out over 1 second (1000 milliseconds)
            await Christmas.FadeTo(0, 1000);

            // Fades the title back in over 1 second
            await Christmas.FadeTo(1, 1000);
        }
